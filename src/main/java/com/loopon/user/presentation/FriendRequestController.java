...
public class FriendRequestController {
    ...
    public CommonResponse<PageResponse<FriendSearchResponse>> findNewFriend(@AuthenticationPrincipal PrincipalDetails principalDetails, @RequestParam String query,
        ...
        return CommonResponse.onSuccess(friendSearchResponse);
    }
    @GetMapping
    @Operation(summary= "친구 요청 목록 조회", description = "회원이 받은 친구 요청 목록을 가져옵니다.")
    public CommonResponse<PageResponse<FriendRequestReceivedResponse>> getFriendRequests(@AuthenticationPrincipal PrincipalDetails principalDetails, Pageable pageable) {
        Long me = principalDetails.getUserId();
        PageResponse<FriendRequestReceivedResponse> page = friendRequestService.getFriendRequests(me, pageable);

        return CommonResponse.onSuccess(page);
    }
    //내가 원하는 친구에게 요청을 보내는 API
    @PostMapping("/send")
    @Operation(summary= "친구 요청 전송", description = "친구 요청을 전송합니다.")
    public CommonResponse<FriendRequestCreateResponse> sendFriendRequest(@AuthenticationPrincipal PrincipalDetails principalDetails,@RequestBody FriendRequestCreateRequest friendRequestRespondRequest) {
        Long me = principalDetails.getUserId();
        FriendRequestCreateResponse  res = friendRequestService.sendFriendRequest(me, friendRequestRespondRequest.receiverId());
        return CommonResponse.onSuccess(res);
    }
    //내가 받은 친구 요청에 대해 수락/거절/차단 API
    @PatchMapping("/respond")
    @Operation(summary= "친구 요청 수락/거절", description = "친구 요청 1개(수락/거절)를 처리합니다.")
    public CommonResponse<FriendRequestRespondResponse> friendRequestResponse(@AuthenticationPrincipal PrincipalDetails principalDetails, @RequestBody FriendRequestRespondRequest friendRequestRespondRequest) {
        Long me = principalDetails.getUserId();
    ...
    public CommonResponse<FriendRequestBulkRespondResponse> friendRequestResponseAll(@AuthenticationPrincipal PrincipalDetails principalDetails, @RequestParam FriendStatus friendStatus) {
        Long me = principalDetails.getUserId();
