...
public interface JourneyApiDocs {
    ...
    @Operation(summary = "여정 이어가기", description = "기존 여정의 목표를 그대로 가져와 새로운 여정 생성을 위한 데이터를 반환합니다")
    @ApiResponse(responseCode = "200", description = "여정 이어가기 데이터 조회에 성공하였습니다.", useReturnTypeSchema = true)
    @CommonBadRequestResponseDocs
    @CommonInternalServerErrorResponseDocs
