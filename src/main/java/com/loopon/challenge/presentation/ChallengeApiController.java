...
public class ChallengeApiController implements ChallengeApiDocs {
    ...
    public ResponseEntity<CommonResponse<ChallengePostResponse>> postChallenge(
        ...
        return ResponseEntity.ok(CommonResponse.onSuccess(challengeCommandService.postChallenge(commandDto)));
    }

    @Override
    @GetMapping("/api/challenges/{challengeId}")
    public ResponseEntity<CommonResponse<ChallengeGetResponse>> getChallenge(
            @PathVariable Long challengeId
    ) {
        return ResponseEntity.ok(CommonResponse.onSuccess(challengeQueryService.getChallenge(challengeId)));
    }

    @Override
    @PostMapping("/api/challenges/{challengeId}/like")
    public ResponseEntity<CommonResponse<ChallengeLikeResponse>> likeChallenge(
            @PathVariable Long challengeId,
        ...
        return ResponseEntity.ok(CommonResponse.onSuccess(challengeCommandService.likeChallenge(commandDto)));
    }

    @Override
    @PatchMapping(value = "/api/challenges/{challengeId}", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<CommonResponse<ChallengeModifyResponse>> patchChallenge(
        ...
        return ResponseEntity.ok(CommonResponse.onSuccess(challengeCommandService.modifyChallenge(commandDto)));
    }

    @Override
    @PostMapping("/api/challenges/{challengeId}/comments")
    public ResponseEntity<CommonResponse<ChallengeCommentResponse>> commentChallenge(
            @PathVariable Long challengeId,
        ...
        return ResponseEntity.ok(CommonResponse.onSuccess(challengeCommandService.commentChallenge(commandDto)));
    }
    ...
    public ResponseEntity<CommonResponse<ChallengeLikeCommentResponse>> likeCommentChallenge(
        ...
        return ResponseEntity.ok(CommonResponse.onSuccess(challengeCommandService.likeCommentChallenge(commandDto)));
    }

    @Override
    @DeleteMapping("/api/challenges/{challengeId}/comments/{commentId}")
    public ResponseEntity<CommonResponse<Void>> deleteCommentChallenge(
            @PathVariable Long commentId,
            @PathVariable Long challengeId,
            @AuthenticationPrincipal PrincipalDetails principalDetails
    ) {
        ...
        return ResponseEntity.ok(CommonResponse.onSuccess(challengeCommandService.deleteCommentChallenge(commandDto)));
    }

    @Override
    @DeleteMapping("/api/challenges/{challengeId}")
    public ResponseEntity<CommonResponse<Void>> deleteChallenge(
            @PathVariable Long challengeId,
            @AuthenticationPrincipal PrincipalDetails principalDetails
    ) {
    ...
    public ResponseEntity<CommonResponse<ChallengeCombinedViewResponse>> viewChallenge(
        ...
        return ResponseEntity.ok(CommonResponse.onSuccess(challengeQueryService.viewChallenge(commandDto)));
    }


    @Override
    @GetMapping("/api/challenges/users/{nickname}/details")
    public ResponseEntity<CommonResponse<SliceResponse<ChallengeDetailResponse>>> detailsChallenge(
            @PathVariable String nickname,
            @PageableDefault Pageable pageable
    ) {
        ChallengeDetailCommand commandDto = ChallengeConverter.detailChallenge(nickname, pageable);
        return ResponseEntity.ok(CommonResponse.onSuccess(challengeQueryService.detailsChallenge(commandDto)));
    }
}
